...
import { AnimationFrameReqBase, AnimationState, AnimationData } from "./AnimationFrameReqBase.js";

export * from "./AnimationFrameReqBase.js";
export class AnimationFrameRequestor extends AnimationFrameReqBase {

    callback!: (currDistance: number) => boolean;
    onCycleComplete!: (currDistance: number) => Promise<boolean>;
    constructor(distance: number | string | null, totalTime: number, callback: (currDistance: number) => boolean) {
        super();
        ...
        if (db) console.log("param.distance=" + distance);
    ...
    async start(initialDelay?: number) {
        ...
        if (db) console.log(`sleeping for ${initialDelay} secs`);
    ...
    //=========================================================================
    lastDistance = 0;
    private requestAnimations() {
        let p = new Promise<void>((resolve, reject) => {
            let stepFunc = async (currTime: DOMHighResTimeStamp) => {
                this.currFrameCount++;

                let pauseStartedTimeSaved = this.pauseStartedTime;
                if (this.currState === AnimationState.Pausing) {
                    this.pauseStartedTime = currTime; //Date.now();
                    if (db) console.log("---pausing, this.pauseStartedTime set to: " + this.pauseStartedTime);
                    resolve(); return; //temporarily stops here. ------------------------------
                }
                else {
                ...
                if (this.currState === AnimationState.Ready) {
                    ...
                    if (db) console.warn(errMesg);
                    if (db) console.log("AnimationState.Finished 2");
                    resolve(); return; //stops here. ------------------------------
                }
                ...
                let data: AnimationData = { currTime: currTime, currDistance: currDistance, elapsedTime: elapsedTime };
                ...
                if (currDistance < this.distance) {
                    ...
                    if (!keepRequestingMoreFrames) {
                        if (db) console.log("stopping, callback return false");
                        resolve(); return; //stops here. ------------------------------
                    }
                    ...
                    if (!keepRequestingMoreFrames) {
                        if (db) console.log("stopping, lastCallback return false");
                        resolve(); return; //stops here. ------------------------------
                    }
                    ...
                    if (this.onCycleComplete) {
                        ...
                        if (keepRunning) {
                            if (this.currState === AnimationState.Running || this.currState === AnimationState.Resuming) {
                                ...
                                if (db) console.log("Running again z...");

                                this.resetForNextCycle();
                                requestAnimationFrame(stepFunc);
                            }
                            else {
                                if (db) console.log("onCycleComplete, keepRunning callback=true, but currState changed to:" + AnimationState[this.currState]);
                                resolve(); return; //stops here. ------------------------------
                            }
                        }
                        else {
                            ...
                            if (db) console.log("AnimationState.Finished 1");
                            resolve(); return; //stops here. ------------------------------
                        }
                    }
                    else {
                        if (db) console.log("stopping, onCycleComplete is null");
                        resolve(); return; //stops here. ------------------------------
                    }
                }
            }; //end stepFunc
