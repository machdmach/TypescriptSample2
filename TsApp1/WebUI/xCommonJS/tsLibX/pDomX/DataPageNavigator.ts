...
import { HTMLAnchorElementX, NumberArray } from "../tsLibPkg.js";
import { ElementSwitch } from "./ElementSwitch.js";
import { ToggleDisplay } from "./ToggleDisplay.js";

export interface DataPageNavigatorEventArgs {
    pageIndex: number;
    pageSize: number;
}

export class DataPageNavigator {
    pageNumberInputText: HTMLInputElement;
    pageSizeSelections: HTMLSelectElement;

    firstPageBtn: ElementSwitch;
    previousPageBtn: ElementSwitch;
    nextPageBtn: ElementSwitch;
    lastPageBtn: ElementSwitch;

    whatsDisplayingInfo: HTMLElement;
    numberOfPagesLabel: HTMLElement;
    pageXOfYDiv: ElementSwitch;
    events: MyEvents;

    pageNav: HTMLElement;
    mainSwitch: ElementSwitch;
    noRowsFoundElt: HTMLElement;

    siblingNav?: DataPageNavigator;
    pageNavContainer: HTMLElement;

    private displayToggle: ToggleDisplay;
    hide() { this.displayToggle.hide(); }
    show() { this.displayToggle.show(); }
    showOnlyForLongPage = false;
    recordsLabel = "records";

    constructor(pageNavContainerEltOrSelector: string | HTMLElement, initialPageSize = 50) {
    ...
    //=========================================================================
    createBottomNavigator(pageNavContainerEltOrSelector: string | HTMLElement) {
        let topNav = this;
    ...
    //=========================================================================
    setSiblingOfEachOther(siblingNav: DataPageNavigator) {
    ...
    //=========================================================================
    createAnchorElementSwitch(textContent: string): ElementSwitch {
        let elt = this.createControl();
        let sw = new ElementSwitch(elt);
        let posElt = sw.createElementPos("a", textContent) as HTMLAnchorElement;
        posElt.href = "#pos";
        ...
        let negElt = sw.createElementNeg("span", textContent) as HTMLAnchorElement;
    ...
    //=========================================================================
    setupEventListeners() {
        let handleEvent = (ev: Event, newPageIndex: number) => {
            console.log("Input event: ", ev);
            let elt = ev.currentTarget;
            console.log("ev.currentTarget element", elt);

            let args: DataPageNavigatorEventArgs = { pageIndex: newPageIndex, pageSize: this.getPageSize() };
            this.events.fireEvent("change", args);
        };

        this.pageNumberInputText.addEventListener("change", (ev: Event) => {
            handleEvent(ev, this.getCurrentPageIndex());
        });

        this.pageSizeSelections.addEventListener("change", (ev: Event) => {
            let sel = ev.currentTarget as HTMLSelectElement;
            if (this.siblingNav) {
        ...
        this.firstPageBtn.eltPos.addEventListener("click", (ev: Event) => {
            handleEvent(ev, 0);
        });
        this.previousPageBtn.eltPos.addEventListener("click", (ev: Event) => {
            handleEvent(ev, this.getCurrentPageIndex() - 1);
        });
        this.nextPageBtn.eltPos.addEventListener("click", (ev: Event) => {
            handleEvent(ev, this.getCurrentPageIndex() + 1);
        });
        this.lastPageBtn.eltPos.addEventListener("click", (ev: Event) => {
            handleEvent(ev, -1);
        });
    }

    //=========================================================================
    appendTextChild(containerElt: HTMLElement, text: string) {
    ...
    //=========================================================================
    setValues(totalRowCount: number, pageIndex: number, numberOfRowsDisplaying: number) {
    ...
    //=========================================================================
    setValues_self(totalRowCount: number, pageIndex: number, numberOfRowsDisplaying: number) {
