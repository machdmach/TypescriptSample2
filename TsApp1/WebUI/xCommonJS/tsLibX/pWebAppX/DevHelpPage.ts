...
import { DetailsSummaryX } from "../pDomX/DetailsSummary.js";
import { MessageBox } from "../pDomX/MessageBox.js";
import { ResourseFetcher } from "../pDomX/ResourceFetcher.js";
import { AppLib } from "./AppLib.js";
import { DevLib } from "./DevLib.js";

//=========================================================================
export class DevHelpPage {
    static remoteDevURL: string;
    static QAURL: string;
    static ProductionURL: string;

    static helpDiv: HTMLElement;
    static isInitDevPage = true;
    static async init() {
        if (alreadyExecuted("DevHelpPage.init")) return;

        if (!this.isInitDevPage) {
            return;
        }
        let html: string = "uninit";
        try {
    ...
    //=========================================================================
    private static setupSidebarDevLinks() {
        let currHref = location.href;
        let currHrefLower = location.href.toLowerCase();

        let ul = document.createElement("ul");
        let addLink = function (href: string | null, textContent: string, className?: string) {
        ...
        zzhelp.addEventListener("click", async (ev: MouseEvent) => {
            ev.stopPropagation();
        ...
        zzShowDebug.addEventListener("click", async (ev: MouseEvent) => {
            // ev.stopPropagation();
            // ev.stopImmediatePropagation();
            ev.preventDefault();
