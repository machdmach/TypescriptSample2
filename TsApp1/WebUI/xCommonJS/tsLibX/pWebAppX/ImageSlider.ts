import { CircularQueue } from "../pSystemX/CircularQueue.js";
import { AnimationFrameRequestor, AnimationState } from "../pDomX/AnimationFrameRequestor.js";
import { ThreadX } from "../pSystemX/ThreadX.js";
import { ImageSliderBase } from "./ImageSliderBase.js";
import { ArrayX } from "../pSystemX/ArrayX.js";
...
export class ImageSlider extends ImageSliderBase {
    ...
    constructor(eltOrSelector: string | HTMLElement) {
        ...
        if ("pause btn") {
            ...
            btn.addEventListener("click", (ev: MouseEvent) => {
                if (db) console.log("pauseBtn clicked: currState:" + this.animator.currStateName);
                this.stopEvent(ev);
                this.toggle(ev);
                //this.pause(ev);
            });
        ...
        if ("play btn") {
            ...
            btn.addEventListener("click", async (ev: MouseEvent) => {
                if (db) console.log("playBtn clicked: currState:" + this.animator.currStateName);
                this.stopEvent(ev);
                this.toggle(ev, 0);
            });
            this.playBtn = btn;
    ...
    setupContainerElt() {
        ...
        this.containerElt.addEventListener("click", (ev: MouseEvent) => {
            if (db) console.log("containerElt clicked");
            this.stopEvent(ev);
            this.toggle(ev);
        ...
        this.containerElt.addEventListener("keyup", (ev: KeyboardEvent) => {
            if (db) console.log("container keyup event: ", ev);
            this.stopEvent(ev);
    ...
    //=========================================================================
    buildElementsFromImages(imageSrcList: string[]) {
        let slides: Element[] = [];
    ...
    //=========================================================================
    setSlides(slides: string[] | Element[]) {
        if (ArrayX.areAllStrings(slides)) {
            slides = this.buildElementsFromImages(slides);
        }
        if (slides.length < 2) {
            if (db) console.log("imageList is too short");
        }
        //this.slidingElt.innerHTML = "loading...";

        this.slideQueue = new CircularQueue<Element>();
    ...
    //=========================================================================
    async jumpToImageWithIndex(slideIndex: number) {
        this.animator.stop();
    ...
    //=========================================================================
    async slideToNextImage(evt: MouseEvent | KeyboardEvent) {
    ...
    //=========================================================================
    async slideToPreviousImage(evt: MouseEvent | KeyboardEvent) {
    ...
    //=========================================================================
    async toggle(evt?: MouseEvent | KeyboardEvent, initialDelay?: number) {
    ...
    async run(initialDelay?: number) {
        let gotoSlideBtnReadyToChange = true;

        let onCycleCompleteFunc = async (currDistance: number) => {

            if (db) console.log("callback received Slide-Finished");
            this.swapImages();

            if (db) console.log("nap time, pausing for " + this.pauseTime + " millis");
        ...
        //--------------------------------------------
        let callbackStepFunc = (currDistance: number) => {
            currDistance = Math.ceil(currDistance);

            if (this.animator.currFrameCount > 10 && gotoSlideBtnReadyToChange) {
                if (db) console.log("updating the the currentSlideButton b/c 10 frames have passed, frameCount=" + this.animator.currFrameCount);
        ...
        if (this.animator !== undefined) {
            if (db) console.log("animator already running");
    ...
    //=========================================================================
    enqueueNewSlides(imageSrcList: string[] | Element[]) {
