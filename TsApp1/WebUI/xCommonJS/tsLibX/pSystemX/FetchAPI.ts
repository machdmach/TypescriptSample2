...
export class FetchAPI {
    ...
    //=========================================================================
    static createRequestInit(): RequestInit {
    ...
    async dispatchReturningJSON(method: string, url: string, formData?: FormData): Promise<any> {
        ...
        if (respondContentType.includes("application/json")) {
            if (db) console.log('response is json');
            let ret: Promise<any> = response.json(); //deserialized to ResponseData
            return ret; //-------------
        }
        else {
            response.text().then((responseText: string) => {
                if (db) console.error("response text: ", responseText);
            });
    ...
    //=========================================================================
    async dispatch(method: string, url: string, formData?: FormData): Promise<Response> {
        ...
        if (formData) {
            ...
            if (db) console.log("Posting FormData", FormDataX.toStr(this.reqInit.body));
            windowExtBag.formDataSubmitted = formData;
        }

        const req = new Request(url, this.reqInit);
        if (db) console.log('start fetchin...', { url, req });

        let promise = fetch(req)
            .then((response: Response) => {
                if (db) console.log('fetchResponse received:', response);
                if (!response.ok) {
