import {
    DocumentX, HColumnSortElementCollection, HTMLElementOverlay, HtmlEntity, HTMLGridBuilder, HTMLTableBuilder, MessageBox,
    AppLocalStorage, NodeTree, RegExpr, ResourseFetcher, StatusBar, StringInterpolation, StringX, TimedThrottleJobQueue
} from "../tsLibPkg.js";
...
export class LibraryTestMain {
    ...
    static async runTests() {
        ...
        if (hrefLower.includes('zz-icon')) MessageBox.showInfo(HtmlEntity.test());

        if ("x") HTMLElementOverlay.tests();

        if ("") TimedThrottleJobQueue.Tests();

        if ("") this.HTMLGridBuilder_test1();
        if ("") NodeTree.grep_tests();
        if ("") AppLocalStorage.test1();

        if ("") HTMLTableBuilder.tests();

        if ("") StringX.format_tests();
        if ("") RegExpr.exec_tests();
        if ("") StringInterpolation.test1();
        if ("") StatusBar.test1();

        if ("") await ResourseFetcher.loadImageBlob(document.body as HTMLImageElement, "flowers.jpg");
        if ("") DocumentX.createDummyDivs(100, qs("main"));
        ...
        if ("") {
            if (alreadyExecuted("x123")) return;
        ...
        if ("") { windowExtBag.MutationObserverX.observeSelector('body'); }
