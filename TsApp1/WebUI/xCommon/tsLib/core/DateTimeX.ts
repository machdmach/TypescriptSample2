...
export class DateTimeX {
    static checkValidity(d: Date, errMesg = "Date is not valid") {
        if (!this.isValid(d)) {
    ...
    //=========================================================================
    static formatDateHHmm(dt?: Date | null) {
        let ret = "";
        if (dt) {
            let dtObj: Date | undefined = undefined;
    ...
    //=========================================================================
    static GetQuarterLabel(q: number, titleCase = true) {
        let ret: string;
        if (q === 1) ret = "first";
        else if (q === 2) ret = "second";
        else if (q === 3) ret = "third";
        else if (q === 4) ret = "fourth";
        else throw Error("Invalid quarter: " + q);
        if (titleCase) {
    ...
    static addUTCHours(dt: Date, hours: number) {
        let dtOrigStr = dt.toUTCString();
        //let dayOfMonth = dt.getUTCDate();
        let newTimeMillis = dt.getTime(); // + (days 24 * 60 * 60 * 1000);
        let rval = new Date(newTimeMillis);
    ...
    //=========================================================================
    static getTimeString(dt: Date, utc = true) {
        //return rval;
        // dt.setUTCHours(2);
        // dt.setMinutes(0);
        // dt.setSeconds(0);
    ...
    //=========================================================================
    static DiffInWeeks(d1: Date, d2: Date): number {
        let t2 = d2.getTime();
        let t1 = d1.getTime();
        return (t2 - t1) / (24 * 3600 * 1000 * 7);
    }
    //=========================================================================
    static diffInMinutes(d1: Date, d2: Date) {
