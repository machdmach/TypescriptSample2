...
export class TimedBinarySemaphore {
    ...
    intervalTimerHandle: number | undefined;
    ...
    tryGet(): boolean {
        if (this.intervalTimerHandle === undefined) {
            this.intervalTimerHandle = window.setInterval(function (p1: any) {
                let thisObj = p1 as TimedBinarySemaphore;
    ...
    callIfANotAlreadyCalledRecently(callback: Function) {
        if (this.tryGet()) {
