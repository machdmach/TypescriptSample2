...
export class XMLHttpRequestX {
    async dispatchReturningJSON(method: string, url: string, formData?: FormData): Promise<any> {
        let xhr: XMLHttpRequest = await this.dispatch(method, url, formData);
    ...
    get(url: string): Promise<XMLHttpRequest> {
        let ret: Promise<XMLHttpRequest> = this.dispatch("GET", url);
        return ret;
    }

    //=========================================================================
    dispatch(method: string, url: string, formData?: FormData): Promise<XMLHttpRequest> {
