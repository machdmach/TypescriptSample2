...
export class XMLHttpRequestX {
    ...
    //=========================================================================
    dispatch(method, url, formData) {
        if (db)
            console.log("XML Http Req url=" + url);
...
*/
        ...
        let promise = new Promise((resolve, reject) => {
            ...
            xhr.onload = () => {
                if (xhr.status !== 200) {
                    if (db)
                        console.error("err: response not OK, failed to fetch url=" + url, xhr);
...
-----------------------------314911788813839--
    */
    ...
    static arrayBufferToBase64String(buffer) {
        let binaryString = '';
