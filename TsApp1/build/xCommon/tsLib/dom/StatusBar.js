...
export class StatusBar {
    ...
    static setTop(ordinate) {
        let st = this.statusBarElt.style;
        st.top = ordinate + "px";
    }
    static async setText(mesg, isPermanent = false) {
        if (!this.isEnabled) {
        ...
        if (location.href.includes("zzstay")) {
            isPermanent = true;
        } //for testing purpose
        if (!isPermanent) {
            setTimeout(() => {
...
//=========================================================================
StatusBar.currentTextMesg = "uninit";
