...
*/
    ...
    //=========================================================================
    static parseDateTimeProperty_eg(obj, propertyName) {
    ...
    static parse_MM_DD_yyyy_hhmm(s) {
        //09/30/2016 2135
        s = s.trim();
        let arr = s.split(" ");
        let d = arr[0];
        ...
        let ret = new Date(this.parseShort(yyyy, "year"), this.parseShort(MM, "month") - 1, this.parseShort(dd, "day"), this.parseShort(hh, "hour"), this.parseShort(mm, "minute"));
        if (!DateTimeX.isValid(ret)) {
            throw Error("invalid date: " + s);
        }
        return ret;
    }
    static parseShort(s, label) {
