...
export class Randomizer {
    ...
    static generateDecimal(min, max, decimalPlaces) {
    ...
    static generateParagraph_withLengths(...lenArr) {
        let i = Math.floor(Math.random() * lenArr.length);
        let len = lenArr[i];
        return this.generateParagraph(len, len);
    }
    static generateParagraph(minLen = 1, maxLen = 1000) {
        if (minLen > maxLen) {
    ...
    static generateWord(minLen = 1, maxLen = 20) {
        if (minLen > maxLen) {
            throw Error("Invalid args, minLen GT maxLen");
        }
    ...
    static generateInteger(min, max = Number.MAX_SAFE_INTEGER) {
